...
const EmailUserupdate = () => {
    ...
    useEffect(() => {
        const url = `http://localhost:5000/single/${emailId}`
        fetch(url)
            .then(res => res.json())
            .then(data => {
                setUpdateUser(data)
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <Form className='w-50 mx-auto text-start'>
                            {updateUser?.name && <Form.Group className="mb-3" >
                                ...
                                <Form.Control readOnly defaultValue={updateUser?.name} {...register("name")} placeholder="Enter Name" />
                            </Form.Group>}
                            {updateUser?.email && <Form.Group className="mb-3" >
                                ...
                                <Form.Control readOnly defaultValue={updateUser?.email}  {...register("email")} placeholder="Enter email" />
                            </Form.Group>}
